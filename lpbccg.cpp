...
// LPBCCG- Level Proximal Bundle Column-and-Constraint generation 
//===================================================
#include <iostream>
#include <iomanip>
#include <fstream>
...
int main (int argc, char *argv[]){
    ...
    try {
        ...
        while(mp.gap > EPSILON and crono.getTime() <= 21600){
            ...
            ++nh;
            printf ("%5d |%5d | %8.2f | %8.2f | %8.4f | %8.2f | %8.2f\n",nh , nobb, mp.lb, mp.ub, 100*mp.gap, mp.TimeMaster,crono.getTime());
        }//endWhile
...
void iniciatilization(DAT &d, MP_CPX_DAT & mp){
    ...
    mp.IsCCG = 1;   mp.IsBundle = 0;  mp.iter = 0; mp._tk = 1; mp._count = -1;     
...
void create_model_mp(DAT &d, MP_CPX_DAT & mp){
    ...
    //mp.cplex.setParam(IloCplex::PreInd,0);    
...
//===============
    ...
    //cout << "Demand Vectors : \n" ;
    //for(int j = 1; j <= d.n; ++j) cout << d.dd[j] << " + " << d.dde[j] <<"\n";
    //cout << " MaxD : " << mp.maxD << "\n" ;
//===============
//obj. function- mpof
//===============
...
void add_constraintssystems(DAT &d, MP_CPX_DAT &mp){    
    ...
    for ( j = 1; j <= d.n; ++j){
        ...
        //  //mpcut1{h in H}: sum{i in I} 10*f[i] * y[i] + sum{i in I} a[i] * z[i] + sum{i in I, j in J} c[i,j] * x[h,i,j] <= a0 * (0.6 * lb + 0.4 * ub) + a1 * 1.00e+12;
    IloExpr r61mpcut(env);
    for (int i = 1; i <= d.m; ++i){
        r61mpcut += 10*d.f[i] * mp.y[i - 1];
        r61mpcut += d.a[i] * mp.z[i - 1];
    }
    double rhs = mp.IsBundle*(0.6*mp.lb + 0.4*mp.ub) + mp.IsCCG* 1.00e+12;
...
void solve_mp(DAT &d, MP_CPX_DAT & mp){ 
    ...
    if (mp.IsCCG == 1 && mp.IsBundle == 0){
        ...
        if (mp.iter >= 8 && mp.IsCCG ==1){
            ...
            mp.cplex.setParam(IloCplex::IntSolLim, 1);           //mipsolutions = 2   
        ...
        if (mp.cplex.getStatus() == IloAlgorithm::Infeasible || mp.cplex.getStatus() == IloAlgorithm::Unknown  || mp.cplex.getStatus() == IloAlgorithm::Unbounded){
            ...
            for(int i = 1; i <= d.m; ++i){
                ...
                // // constraintssystems3 constraint /bound a0*(0.6*mp.lb + 0.4*mp.ub) + a1* 1.00e+12;
            for(int i = 1; i <= mp._rbs.getSize(); ++i){
                mp._rbs[i-1] = mp.IsBundle*(0.6*mp.lb + 0.4*mp.ub) + mp.IsCCG*1.00e+12;
            }
            ...
            mp.cplex.getValues(mp._vars,mp.vars);
            for(int i = 0; i < d.m; ++i){
                mp._z[i] = abs(mp._vars[i]);
                mp._y[i] = abs(mp._vars[d.m+i]);
            }
        }
    }
}
...
//===========
        FILE *arquivo1;
        arquivo1=fopen("LPBCCG-Result.txt","aw+");
        if(arquivo1 == NULL){
            printf("Não foi possivel abrir o arquivo1");
            exit(0);
        }
        fprintf (arquivo1,"%2s %2s %1s %5d %2s %1s %5d %2s %1s %10.4f %2s %1s %10.4f %2s %1s %8.4f %2s %1s %8.4f %2s %1s %8.4f %2s %1s  %8.4f\n",d.name,"&","&", h,"&","&", nb,"&","&", mp.lb,"&","&", mp.ub,"&","&", Gp,"&","&", mp.TimeMaster,"&" ,"&" ,TempoporsegMaster, "&", "&" ,Tp);
