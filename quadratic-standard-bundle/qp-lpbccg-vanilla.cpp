...
void iniciatilization(DAT &d, MP_CPX_DAT & mp){
    ...
    mp.IsCCG = 0;   mp.IsBundle = 1;  mp.iter = 0; mp._tk = 1; mp._count = -1;     
...
void add_constraintssystems(DAT &d, MP_CPX_DAT &mp){    
    ...
    double rhs = mp.lb + mp.IsBundle*mp.Lk*(mp.ub - mp.lb) + mp.IsCCG* 1.00e+12;
        ...
        if (mp.cplex.getStatus() == IloAlgorithm::Infeasible || mp.cplex.getStatus() == IloAlgorithm::Unknown  || mp.cplex.getStatus() == IloAlgorithm::Unbounded){
            ...
            else 
            {
                ...
                for(int i = 1; i <= mp._rbs.getSize(); ++i){
                    mp._rbs[i-1] = mp.lb + mp.IsBundle*mp.Lk*(mp.ub - mp.lb) + mp.IsCCG* 1.00e+12;
                }
...
//===========
        FILE *arquivo1;
        arquivo1=fopen("QP-LPBCCG-Result-Vanilla.txt","aw+");
        if(arquivo1 == NULL){
