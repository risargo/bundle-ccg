...
int main (int argc, char *argv[]){
    ...
    try {
        ...
        while(mp.gap > EPSILON and crono.getTime() <= 21600){
            ...
            ++mp.iter;
            ++nh;
            //printf ("%5d |%5d | %8.2f | %8.2f | %8.4f | %8.2f\n",nh , nobb, mp.lb, mp.ub, 100*mp.gap, crono.getTime());
...
void iniciatilization(DAT &d, MP_CPX_DAT & mp){
    ...
    mp.IsCCG = 1;   mp.IsBundle = 0;  mp.iter = 0;
}
// ==============================
// Function mp problem
// ==============================
...
void add_constraintssystems(DAT &d, MP_CPX_DAT &mp){    
    ...
    double rhs = mp.IsBundle*(0.6*mp.lb + 0.4*mp.ub);
...
void solve_mp(DAT &d, MP_CPX_DAT & mp){ 
    ...
    if (mp.IsCCG == 1 && mp.IsBundle == 0){
        ...
        for( i = 1; i <= d.m; ++i){
            ...
            mp.mpof.setQuadCoef(mp.e[i-1], mp.e[i-1], 0.5*mp.IsBundle);
        }
            //  //solve 
        ...
        if (mp.cplex.getStatus() == IloAlgorithm::Infeasible || mp.cplex.getStatus() == IloAlgorithm::Unknown  || mp.cplex.getStatus() == IloAlgorithm::Unbounded){
            ...
            for( i = 1; i <= mp._rbs.getSize(); ++i){
                ...
                mp._rbs[i-1] = mp.IsBundle*(0.6*mp.lb + 0.4*mp.ub);
            }
...
//===========
        FILE *arquivo1;
        arquivo1=fopen("QP-LBCCG-Result.txt","aw+");
        if(arquivo1 == NULL){
            printf("Não foi possivel abrir o arquivo1");
            exit(0);
        }
        fprintf (arquivo1,"%2s %2s %1s %5d %2s %1s %5d %2s %1s %10.4f %2s %1s %10.4f %2s %1s %8.4f %2s %1s %8.4f %2s %1s %8.4f %2s %1s \n",d.name,"&","&", h,"&","&", nb,"&","&", mp.lb,"&","&", mp.ub,"&","&", Gp,"&","&", TempoporsegMaster,"&" ,"&",Tp, "&", "&");
