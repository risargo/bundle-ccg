    ...
    try {
        ...
        while(mp.gap > EPSILON and crono.getTime() <= 21600){
            if (mp.IsCCG == 1 && mp.IsBundle == 0){
                ...
                //  //printf ("Target: %10.4f  ", 0.5*(mp2.lb + mp2.ub));
                // ===========
                // mp
                // ===========
                mp.flag == 1;
...
void create_model_mp(DAT &d, MP_CPX_DAT & mp){
    ...
    mp.cplex.setParam(IloCplex::Param::TimeLimit, 21600);   //mipgap    
...
//===============
    ...
    for(int i = 1; i <= d.m; ++i){
       ...
       xpfo += mp.IsBundle * mp.e[i - 1] * mp.e[i - 1] ; 
    }
...
void solve_mp(DAT &d, MP_CPX_DAT & mp){ 
    ...
    mp.cplex.setParam(IloCplex::Param::TimeLimit, 21600);
    mp.cplex.solve();
...
//===========
        FILE *arquivo1;
        arquivo1=fopen("QP-LPBCCG-Result.txt","aw+");
        if(arquivo1 == NULL){
